...
//
// 4.通过PC打开无线转串口 USB端的串口  串口波特率为 zf_device_ble6a20.h 文件中 BLE6A20_BUAD_RATE 宏定义 默认 115200，核心板按下复位按键
//
...
void main(void)
{
    ...
    {
        // 此处编写需要循环执行的代码
        data_len = ble6a20_read_buffer(data_buffer, 32);                    		// 查看是否有消息 默认缓冲区是 BLE6A20_BUFFER_SIZE 总共 64 字节
