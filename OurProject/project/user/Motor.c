...
void Motor_Init(void)
{
    ...
    gpio_init(DIR_1, GPO, 1, GPO_PUSH_PULL);//默认高电平为正转
    gpio_init(DIR_2, GPO, 1, GPO_PUSH_PULL);//默认高电平为正转
...
void motor1_control(int16 motor_duty)
{
   if(motor_duty >= 0)                                                          
   {
        gpio_set_level(DIR_1, 1);//需确认                                   
        pwm_set_duty(PWM_1, motor_duty);                   
	}
	else 
	{
		gpio_set_level(DIR_1, 0);//需确认                                   
      pwm_set_duty(PWM_1, -motor_duty);  
	}
...
void motor2_control(int16 motor_duty)
{
   if(motor_duty >= 0)                                                          
   {
        gpio_set_level(DIR_2, 1);//需确认                                   
        pwm_set_duty(PWM_2, motor_duty);                   
	}
	else 
	{
		gpio_set_level(DIR_2, 0);//需确认                                   
      pwm_set_duty(PWM_2, -motor_duty);  
	}
