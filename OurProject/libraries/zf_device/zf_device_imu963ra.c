...
float imu963ra_transition_factor[3] = {4098, 14.3, 3000};
	...
	//-------------------------------------------------------------------------------------------------------------------
	//  @brief      模拟 SPI 全双工：移出 byte 的同时采样 MISO，返回移入的 8 位数据
	//  @param      byte        待移出的数据（高位先出）
	//  @return     uint8       MISO 上采样得到的 8 位数据（非总线 status 寄存器）
	//  @since      v1.0
	//  Sample usage:
	//-------------------------------------------------------------------------------------------------------------------
	...
	//-------------------------------------------------------------------------------------------------------------------
	//  @brief      将 val 写入 cmd 指定的寄存器（片选内两次 simspi 字节交换）
	//  @param      cmd         命令字节（调用方已拼好读写位，如 reg|SPI_W）
	//  @param      val         待写入的数据
	//  @since      v1.0
	//  Sample usage:
	//-------------------------------------------------------------------------------------------------------------------
	...
	//-------------------------------------------------------------------------------------------------------------------
	//  @brief      连续读取 cmd 起始寄存器的 num 个字节到缓冲区 *val（首字节为命令后的第一字节数据）
	//  @param      cmd         命令字节
	//  @param      *val        输出缓冲区
	//  @param      num         读取字节数
	//  @since      v1.0
