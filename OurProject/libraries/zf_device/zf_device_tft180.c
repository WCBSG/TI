...
uint8 tft180_x_max = TFT180_LEN;
uint8 tft180_y_max = TFT180_WIGHT;
...
{
    uint16 i;
    uint16 tmp[TFT180_LEN];
    tft180_set_region(0,0,tft180_x_max-1,tft180_y_max-1);
	
    for(i = 0; i < TFT180_LEN; i ++)
    {
        tmp[i] = color;
    }
	for(i = 0; i < TFT180_WIGHT; i ++)
    {
        tft180_write_16bit_data_arry(tmp, TFT180_LEN);
    }
}
