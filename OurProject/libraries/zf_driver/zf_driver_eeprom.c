...
void eeprom_trig(void)
{
    ...
    _nop_();   //由于STC32G144K是多级流水线的指令系统，触发命令后建议加4个NOP，保证IAP_DATA的数据完成准备
    _nop_();
    _nop_();
    _nop_();
...
// 返回参数     uint8		读取到的值
// 备注信息
// 使用示例   	dat = iap_read_byte(0);	// 向0地址读取一个数据到dat
//-------------------------------------------------------------------------------------------------------------------
...
// 参数说明     addr 		需要读的地址
// 返回参数     uint8		读取到的值
// 使用示例   	iap_write_byte(0x00, 0x33);	// 向0地址写入一个0x33
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     EEPROM读取多个字节，
// 参数说明     addr			需要读取的eeprom地址
// 参数说明     *buf			需要读取的数据地址
// 返回参数     void
// 备注信息
// 使用示例   	iap_read_buff(0x00, buff, 10);	// 向0地址读取一个数组，数组长度为10
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     EEPROM写多个字节，
// 参数说明     addr			需要写的eeprom地址      、
// 参数说明     *buf			需要写的数据地址
// 参数说明     len				需要写的数据长度
// 返回参数     void
// 备注信息
// 使用示例   	iap_read_buff(0x00, buff, 10);	// 向0地址写入一个数组，数组长度为10
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     EEPROM擦除目标地址所在的一页（1扇区/512字节），
// 参数说明     addr			需要写的eeprom地址      
// 返回参数     void
// 备注信息
// 使用示例   	iap_erase_page(0); // 擦除0x00到0x100区域
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     扩展EEPROM写多个字节(无需擦除)，
// 参数说明     addr			需要写的eeprom地址   
// 参数说明     *buf			需要写的数据地址
// 参数说明     len				需要写的数据长度
// 返回参数     void
// 备注信息
// 使用示例   					extern_iap_write_bytes(0x0000,(uint8 *)"0123456789",10);
