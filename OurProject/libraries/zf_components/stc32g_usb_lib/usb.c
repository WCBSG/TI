...
extern void (*usb_cdc_read_buffer_handler)(BYTE *buff, WORD len);
...
void usb_out_ep1()
{
    ...
    if (csr & OUTOPRDY)
    {
        ...
        {
            rx_buff[idx] = usb_read_reg(FIFO1);
        }
        
        // 这里暂时不做USB OUT FULL或者FIFO FULL判断。
        if(usb_cdc_read_buffer_handler)
        {
            usb_cdc_read_buffer_handler(rx_buff, cnt);
        }
        // OUT数据包接收完成标志位，设置为0，即可接收下一包数据。
